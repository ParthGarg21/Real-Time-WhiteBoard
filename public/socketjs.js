...
// setting up socket in the frontend
...
function handleDrawTrigger(event) {
  ...
  // reseting the path for each and every user
...
function handleDraw(event) {
  if (isDraw) {
    ...
    // whenever we want to draw, we'll call this for every user by sending the coordinates and calling the drawhelper function

    socket.emit("drawHelper", data);
...
function handleDrawTurnoff() {
  ...
  // equating the strokes array for each user.
  
  socket.emit("equate", allStrokes);
}

function drawHelper(data) {
...
function handleCilck() {
  ...
  if (optionName === "pencil") {
    ...
    // calling the redo function for each user upon clicking for each user
    socket.emit("triggerUndo", "");
  } else if (optionName === "redo") {
    // calling the undo function for each user upon clicking for each user

    socket.emit("triggerRedo", "");
  } else {
    downloadBoard();
  }
}
...
function colorWidthSetter(data) {
  pen.lineWidth = data.width;
  pen.strokeStyle = data.color;
}

function equateStorkes(data) {
  allStrokes = data;
}
...
socket.on("colorW", function (data) {
  colorWidthSetter(data);
});
